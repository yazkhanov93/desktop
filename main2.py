...
class main:
    ...
    def uploadProduct(self):
        try:
            ...
            df = df.astype({"code":"str","title":"str","description":"str","model":"str","years":"str","motor":"str","size":"str","company_name":"str", "company_part_number":"str", "original_part_number":"str","weight":"str","cascade":"str"}) #, "quantity":"int"})
            ...
            df["main"] = df["main"].replace(1, True)
            df["main"] = df["main"].replace(0, False)
            df["main"] = df["main"].replace("", False)
    ...
    def login(self):
        ...
        if result:
            ...
            self.button2_2 = Button(text="Загрузить", font=("Arial, 11"), width=12, command=self.uploadComp).grid(column=2,
                                                                                                               ...
                                                                                                               padx=10,
                                                                                                               pady=10)
            # self.label = Label(text="Загрузить Другие Бренды", font=("Arial, 16")).grid(column=3, row=3, padx=30, pady=30)
            # self.button3 = Button(text="Выберите Файл", command=self.openfile3, font=("Arial, 11"), width=12).grid(
            #     column=3, row=4, padx=10, pady=30)
            # self.button3_3 = Button(text="Загрузить", font=("Arial, 11"), width=12, command=self.dquality).grid(column=3,
            #                                                                                                    row=5,
            #                                                                                                    padx=10,
            #                                                                                                    pady=10)
